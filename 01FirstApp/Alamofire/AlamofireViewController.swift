...
class AlamofireViewController: UIViewController {
    ...
    func sendReuest() {
        ...
        AF.request(url, method: .post, parameters: parameter ,encoding: URLEncoding.httpBody, headers: ["Content-Type" : "application/x-www-form-urlencoded", "Accept" : "application/json"]).validate(statusCode: 200 ..< 300).responseJSON(completionHandler: {
            (response) in
            print(response)
            // JSON 결과값을 파싱해서 사용한다.
    ...
    func parseJSON(_ response: AFDataResponse<Any>) {
        ...
        case .failure(let error) :
            print("통신 실패 : ", String(describing: error))
            break
        }
    }
